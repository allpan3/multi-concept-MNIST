...
class Bottleneck(nn.Module):
    ...
    @staticmethod
    def flattened_children(model: nn.Module):
        result = []
    ...
    def forward(self, x):
        ...
        if not self.quantized:
            ...
            for child in self.conv1, self.conv2, self.conv3:
                if isinstance(child, nn.Linear) or isinstance(child, nn.Conv2d):
                    scale *= child.weight.scale * child.output_scale

            if self.downsample is not None:
                for child in Bottleneck.flattened_children(self.downsample):
                    if isinstance(child, nn.Linear) or isinstance(child, nn.Conv2d):
                        scale /= child.weight.scale * child.output_scale

            self.add_scale = scale

            identity_scaled = (identity * scale).round()

            out = torch.clamp(identity_scaled + out, min=-128, max=127)
